...
- Groups feature flag: `VITE_GROUPS_ENABLED`, a build-time Vite env var read once into `GROUPS_ENABLED` in `services/config.ts`, gates every Groups-dependent code path — the nav tab, Discover's browse-scope UI and the `p_group_ids` param it would otherwise send, `AddEditItem.tsx`'s Visibility section and the `isPublic` field, and the Groups routes. It exists because Barter2 doesn't have the Groups schema yet, and PostgREST does exact signature matching — sending an unrecognized parameter fails the whole call rather than degrading gracefully. `false`/unset on Barter2, `true` on Barter-dev and local `.env`; a value change needs a genuine fresh build to take effect.
...
- `GroupJoin.tsx`: `/join/:token`, a standalone unauthenticated route outside `Layout`/`ProtectedRoute` — previews the group via `get_group_invite_preview`, then, for a visitor who isn't signed in, stashes the join intent through signup the same way `ProtectedRoute` already stashes an intended path for other deep links, and auto-joins (`join_group_via_link`) on return
