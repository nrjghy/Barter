...
- Groups: a user-created, invite-only circle (`groups` / `group_memberships`, role `creator`/`moderator`/`member` / `item_groups` / `group_invite_links`). A listing's visibility is `items.is_public` plus any number of `item_groups` rows simultaneously — overlapping, not exclusive, so a listing can be public and shared to several groups at once. All writes go through RPCs (`create_group`, `invite_to_group`, `accept_group_invite`/`decline_group_invite`, `transfer_group_ownership`, `leave_group`, `remove_group_member`, `delete_group`, `set_item_groups`, `update_browse_scope`, `set_group_moderator`/`remove_group_moderator` (creator-only), `create_group_invite_link`/`revoke_group_invite_link`/`list_group_invite_links` (creator or moderator; creating a link auto-revokes any other active one, at most one per group), `join_group_via_link`, `get_group_invite_preview` (the one function grantable to `anon`)) — none of the tables has a direct client INSERT/UPDATE/DELETE policy, matching the `trade_completions`/`offers` pattern. `transfer_group_ownership` now takes the new owner's user ID directly, not a username. RLS on `group_memberships` uses a `SECURITY DEFINER` `is_member_of_group()` helper rather than a raw self-referencing policy. `invite_to_group` accepts a username or email and deliberately returns one identical error for a genuine non-match, a demo/system account, and a currently-banned account — none of the three is distinguishable from outside, preserving the same enumeration-prevention property the original username-only design had. Usernames are now enforced unique case-insensitively via a unique index on both databases (with a carve-out for the "Deleted User" placeholder), added after a live duplicate silently misdirected an invite; `Register.tsx` surfaces a friendly collision message on conflict. Invite consumption (accept/decline) is tracked via the notification's own `type` flipping to `group_invite_accepted`/`group_invite_declined`, not `read_at` — an earlier version used `read_at` and broke, since the notification panel's own "mark all read on open" behavior silently consumed it before a user could click. `users.browse_mode` (`public`/`groups`) and `browse_group_ids` persist a user's last Discover scope selection across sessions; both `leave_group` and `remove_group_member` scrub the departing/removed member's own items out of the group's `item_groups` and out of their own `browse_group_ids`. `get_items_browse` takes `p_group_ids uuid[]`: `NULL` is the public feed (unchanged), an array (including empty) scopes to those groups regardless of `is_public`. `record_response_optimized`/`check_and_create_match` both re-check group/public visibility server-side too, despite being `SECURITY DEFINER` and technically able to bypass RLS, closing a real gap where an outsider could otherwise like/pass a private group item they could never actually see. `group_invite_links.group_id` now cascades on delete, like `group_memberships`/`item_groups` already did — it didn't originally, so `delete_group` failed with a foreign-key violation (surfaced to the user as a generic "Failed to delete group") for any group that had ever had an invite link created, even a revoked one, since revoking only sets `revoked_at` and leaves the row in place; found live September 2, 2026 and fixed the same day.
- Groups feature flag: `VITE_GROUPS_ENABLED`, a build-time Vite env var read once into `GROUPS_ENABLED` in `services/config.ts`, gates every Groups-dependent code path — the nav tab, Discover's browse-scope UI and the `p_group_ids` param it would otherwise send, `AddEditItem.tsx`'s Visibility section and the `isPublic` field, and the Groups routes. Originally existed because Barter2 lacked the Groups schema and PostgREST does exact signature matching — sending an unrecognized parameter fails the whole call rather than degrading gracefully. That's no longer the reason: Barter2 now has the full Groups schema (applied September 2, 2026, live-verified via production smoke test). The flag is kept on regardless, now serving as a same-build kill switch rather than a schema-mismatch guard. `true` on both Barter2 and Barter-dev, and local `.env`; a value change needs a genuine fresh build to take effect.
...
netlify/
├─ edge-functions/          # item-preview (migrated off Supabase Aug 27 — Supabase Edge Functions rewrite text/html responses to text/plain on GET, which silently broke its redirect), group-invite-preview (modeled on item-preview but initially copied the self-redirect mistake item-preview had already avoided — see GroupJoin.tsx note below)
public/
...
- `groups.creator_id` always mirrors whichever `group_memberships` row currently has `role = 'creator'` — kept in sync by `transfer_group_ownership`, not a derived/computed value

## Frontend

### Navigation

Bottom-nav tabs — Discover, My Stuff, Chat, Groups (the fourth primary tab,
gated by `GROUPS_ENABLED` — see the feature flag above), plus the Profile
sub-tree (Profile, Account, Notification Settings) — all show the persistent
bottom nav (`BottomNavigation`, gated by its own `showBottomNav` check in
`Layout.tsx`, separate from the narrower three-path check that still governs
the persistent header and top padding, since the Profile sub-tree already
manages its own `BackBar` and spacing). Groups is no longer reached through
Profile — its own tab is the only entry point now, and the Profile menu's
old "Groups" entry was removed to avoid duplicate entry points. The bar is
scroll-aware everywhere it shows — hides sliding down on scroll-down,
reappears on scroll-up even a small amount, always visible near the top of
the page's content — matching common mobile-app convention (Instagram,
Gmail). Every other detail/task screen (item detail, chat threads, the
add/edit form, etc.) still drops both the header and the bottom nav,
replaced by `BackBar` alone. Profile lives behind the header avatar; the
Admin console is reached from Profile's settings menu, not a nav tab.
...
- `Groups.tsx`: the user's groups hub — now a primary tab, not reached through Profile (see Navigation above) — list of groups belonged to, a Create action, and a Pending Invites section (unread `group_invite` notifications, Accept/Decline inline) surfaced here directly, not just in the general notification panel. A one-time dismissible `OnboardingHint` orients first-time visitors, and the zero-groups empty state is actionable rather than a dead end — an inline "Create a group" link alongside guidance to wait for an invite
- `GroupDetail.tsx`: a single group's member list, each row showing a Crown (creator) or ShieldCheck (moderator) role icon next to the member's name, tap-through to `/user/:userId`; Invite offers both plain Invite and Invite via link (`GroupInviteLinkModal`, wrapping `create_group_invite_link`/`revoke_group_invite_link`/`list_group_invite_links`); creator or moderator gets Remove member (per row) and moderator promote/demote actions — a moderator's own controls are hidden on rows the RPC would reject anyway (e.g. against the creator or another moderator), rather than surfacing a server error; creator-only gets Transfer ownership (via a member picker, `TransferOwnershipModal`, not free-text — see `transfer_group_ownership` above) and Delete group (type-the-group-name confirm); non-creator members get Leave instead. A creator can't leave directly, must transfer first — tapping Leave as creator routes straight into the transfer flow rather than a dead-end error. The page's `helpItems` sheet content is computed from the viewer's role
- `GroupJoin.tsx`: rendered at two routes, `/join/:token` (the actual shared link, what `share.ts` generates) and `/g/:token` (client-only, not linked to directly anywhere) — a standalone unauthenticated route outside `Layout`/`ProtectedRoute` — previews the group via `get_group_invite_preview`, then, for a visitor who isn't signed in, stashes the join intent through signup the same way `ProtectedRoute` already stashes an intended path for other deep links, and auto-joins (`join_group_via_link`) on return. The `/g/:token` route exists solely as the `group-invite-preview` edge function's redirect target: that function is itself bound to `/join/:token`, so redirecting back to that same path re-triggered the function on every browser follow-through — an infinite reload loop, found live via a real invite link (fixed September 2, 2026). `join_group_via_link` notifies both sides of the join: a "New member joined" notification (unread) for the link's creator, and a "Group joined" confirmation (pre-read, matching `accept_group_invite`'s equivalent for the invite-by-name/email path) for the joiner — the joiner side was added the same day, after live testing found it missing entirely.
...
- Dialogs: `IssueReportDialog` (now reached from Profile's settings menu, plus a secondary link inside `HelpSheet`, rather than directly behind `Header`'s help icon), `NotificationCenter` (tap-to-navigate is generic, driven by each notification's `actionPath`, not a hardcoded per-table switch; auto-marks-read on open; `product_update` is a plain non-navigating broadcast type), `LocationSharePicker` (current location or search-and-pick a place, unfiltered/precise results, see Location above), `CreateGroupModal`, `InviteToGroupModal`, `TransferOwnershipModal`, `GroupInviteLinkModal`
- Guidance: `InfoTooltip` (viewport-clamped info popover, 12 placements across the app), `OnboardingHint` (one-time dismissible per-tab orientation hint, tracked server-side, now five instances rather than three), `HelpSheet` (contextual help sheet, now the target of `Header`'s "?" icon)
...
Toasts render bottom-center, moved off top-center after it was found to
collide with page content. Duration is per-type: success 2.5s, error 4.5s,
and the new neutral `toastInfo()` at 3.5s. `useToasterStore` caps visible
toasts at 2, so a burst doesn't stack the screen.
...
- `useGroups.ts`: `useGroups()` (the user's own groups list), `useGroup(groupId)` (single group + members + create/invite/transfer/leave/delete/removeMember mutations), `useGroupInviteActions()` (accept/decline, shared between `NotificationCenter` and `Groups.tsx`'s pending-invites section rather than duplicated)
...
- `groupService.ts`: wraps all nine group RPCs plus the read-only groups/members/item-groups queries, same two-layer error-handling convention as the rest of the service layer
