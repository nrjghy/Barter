...
Bottom-nav tabs — Discover, My Stuff, Chat, plus the whole Profile sub-tree
(Profile, Account, Notification Settings, Groups, Group Detail) — all show
the persistent bottom nav (`BottomNavigation`, gated by its own
`showBottomNav` check in `Layout.tsx`, separate from the narrower three-path
check that still governs the persistent header and top padding, since the
Profile sub-tree already manages its own `BackBar` and spacing). The bar is
...
- `Groups.tsx`: the user's groups hub — list of groups belonged to, a Create action, and a Pending Invites section (unread `group_invite` notifications, Accept/Decline inline) surfaced here directly, not just in the general notification panel
- `GroupDetail.tsx`: a single group's member list (creator badge, tap-through to `/user/:userId`), Invite, and, creator-only, Remove member (per row), Transfer ownership, and Delete group (type-the-group-name confirm) actions; non-creator members get Leave instead. A creator can't leave directly, must transfer first — tapping Leave as creator routes straight into the transfer flow rather than a dead-end error
