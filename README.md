...
supabase/
├─ functions/               # Edge functions (send-notification-email, reverse-geocode, places-autocomplete)
...
- **Post-login redirect doesn't preserve intended destination**: `ProtectedRoute.tsx` sends any unauthenticated visitor to `/login` unconditionally, with no memory of where they were headed. This means `item-preview`'s redirect-into-the-app (and any other deep link) works correctly up to the login wall, but someone who actually logs in from there lands on the app's default screen, not the item/page they clicked through for. Not built, flagged for a later pass.
