...
- `useOffers.ts` (also exports `useOffersByIds`), `useTradeCompletions.ts`, `useMessages.ts`, `useNotifications.ts`, `useReports.ts`, `useReviews.ts`, `useUserBlocks.ts`: feature-specific data hooks backed by services
