/*
  # Create Demo Marketplace Environment
  ...
  2. Demo Data
    - 20 realistic user profiles with proper UUIDs
    - 100+ demo items across various categories
...
DO $$
DECLARE
  user_01_id uuid := gen_random_uuid();
  user_02_id uuid := gen_random_uuid();
  user_03_id uuid := gen_random_uuid();
  user_04_id uuid := gen_random_uuid();
  user_05_id uuid := gen_random_uuid();
  user_06_id uuid := gen_random_uuid();
  user_07_id uuid := gen_random_uuid();
  user_08_id uuid := gen_random_uuid();
  user_09_id uuid := gen_random_uuid();
  ...
  user_20_id uuid := gen_random_uuid();
BEGIN
  -- Insert demo users
  ...
  (user_19_id, 'RetroRider_Max', 'Highland Park, Seattle, WA', 'https://images.pexels.com/photos/1040882/pexels-photo-1040882.jpeg?auto=compress&cs=tinysrgb&w=150&h=150&fit=crop', 'Vintage bicycle enthusiast and bike mechanic. I trade classic bikes, parts, and cycling accessories. Love restoring old bikes to their former glory and getting people back on two wheels. 🚲⚙️', true, 4.6, 16, 47.5324, -122.3187, NOW() - INTERVAL '6 months'),
