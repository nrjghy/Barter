...
-- Messages policies (with proper checks)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can read messages from their matches'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'messages' 
    AND policyname = 'Users can send messages to their matches'
  ) THEN
...
-- Swipes policy (with proper check)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    ...
    AND policyname = 'Users can manage their own swipes'
  ) THEN
...
BEGIN
  ...
  INSERT INTO users (id, username, location, avatar_url, bio, is_demo, rating, total_ratings, latitude, longitude, created_at) 
  SELECT * FROM (VALUES
  ...
  ) AS demo_users(id, username, location, avatar_url, bio, is_demo, rating, total_ratings, latitude, longitude, created_at)
  WHERE NOT EXISTS (SELECT 1 FROM users WHERE users.id = demo_users.id);

  -- Insert demo items (only if users exist and items don't already exist)
  INSERT INTO items (user_id, title, description, category, condition, price, image_url, tags, created_at) 
  SELECT * FROM (VALUES
  ...
  ) AS demo_items(user_id, title, description, category, condition, price, image_url, tags, created_at)
  WHERE EXISTS (SELECT 1 FROM users WHERE users.id = demo_items.user_id)
  AND NOT EXISTS (
    SELECT 1 FROM items 
    WHERE items.user_id = demo_items.user_id 
    AND items.title = demo_items.title
  );

END $$;

-- Add a constraint that only applies to future real users (not demo users)
-- This allows demo users to exist without auth.users entries while requiring real users to have them
CREATE OR REPLACE FUNCTION check_user_auth_constraint()
RETURNS TRIGGER AS $$
BEGIN
  -- Only enforce the constraint for non-demo users
...
-- Create trigger to enforce the constraint (with proper check)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'enforce_user_auth_constraint'
  ) THEN
    CREATE TRIGGER enforce_user_auth_constraint
      BEFORE INSERT OR UPDATE ON users
      FOR EACH ROW
      EXECUTE FUNCTION check_user_auth_constraint();
  END IF;
END $$;
