-- Replaces _bootstrap_item_interest_connection (added in
-- 20260818082652_curator_connection_shared_bootstrap.sql) with an
-- OUT-parameter version, matching the sort_user_pair-style convention
-- already used elsewhere in this codebase, and fixes two grant gaps:
--
-- 1. The old helper was left with default privileges (PUBLIC/anon/
--    authenticated could all EXECUTE it), unlike the other internal
--    _-prefixed helpers (_offer_create_core, _complete_trade_core), which
--    are revoked from everyone but postgres/service_role. Same fix here.
-- 2. create_curator_connection itself was missing the
--    REVOKE ... FROM PUBLIC, anon / GRANT ... TO authenticated pair that
--    create_giveaway_connection already has (see
--    20260811145619_create_giveaway_connection_and_branch_response.sql) --
--    it was left anon-callable by oversight.
DROP FUNCTION IF EXISTS public._bootstrap_item_interest_connection(uuid, uuid, uuid);

CREATE FUNCTION public._bootstrap_one_directional_connection(
  p_interested_user_id uuid,
  p_lister_user_id uuid,
  p_item_id uuid,
  OUT the_connection_id uuid,
  OUT is_new_connection boolean,
  OUT already_registered boolean
)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public._bootstrap_one_directional_connection(uuid, uuid, uuid) FROM public;
REVOKE ALL ON FUNCTION public._bootstrap_one_directional_connection(uuid, uuid, uuid) FROM anon;
REVOKE ALL ON FUNCTION public._bootstrap_one_directional_connection(uuid, uuid, uuid) FROM authenticated;

-- Pure refactor -- external behavior/signature unchanged, only the call
-- into the (now OUT-param) shared helper changes shape.
...
begin
  ...
  -- SELECT * (not naming the output columns) deliberately -- they're the
  -- same identifiers as this function's own declared variables, and naming
  -- them explicitly in the select-list is ambiguous in plpgsql (variable
  -- vs. column shadowing).
  select *
...
-- Same OUT-param call-shape update as create_giveaway_connection above.
-- Also now revoked from PUBLIC/anon, matching create_giveaway_connection's
-- own grants -- it was missing that pair since its original migration.
...
REVOKE EXECUTE ON FUNCTION public.create_curator_connection(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_curator_connection(uuid, uuid) TO authenticated;
