-- Groups feature: extend get_items_browse with group-scoped browsing.
-- NULL p_group_ids = Public mode (today's is_public=true behavior,
-- unchanged). Non-null (even empty) array = My Groups mode: items in
-- item_groups for any of those group IDs, ignoring is_public entirely
-- for that mode. Empty array yields zero rows, a valid "nothing
-- checked" state, no special-casing needed since ANY('{}') is always
-- false.
...
CREATE FUNCTION public.get_items_browse(
  ...
  p_include_unrated boolean DEFAULT true,
...
AS $function$
  ...
  matched AS (
    ...
    WHERE i.is_active = true
      ...
      AND (
        (p_group_ids IS NULL AND i.is_public = true)
        OR (p_group_ids IS NOT NULL AND EXISTS (
          SELECT 1 FROM public.item_groups ig
          WHERE ig.item_id = i.id AND ig.group_id = ANY(p_group_ids)
        ))
      )
...
GRANT EXECUTE ON FUNCTION public.get_items_browse(double precision, double precision, integer, text[], text[], uuid, numeric, numeric, integer, numeric, boolean, uuid[], integer, integer) TO PUBLIC;
